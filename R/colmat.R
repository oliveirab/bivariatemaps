...
#' @param nbreaks numeric variable for number of quantiles in color matrix
...
# colmat<-function(nbreaks=10, upperleft="blue", upperright="red", bottomleft="grey", bottomright="yellow", xlab="x label", ylab="y label"){
#   my.data<-seq(0,1,.01)
#   my.class<-classInt::classIntervals(my.data,n=nbreaks,style="quantile")
...
colmat <- function(nbreaks = 10, breakstyle = "quantile",
  ...
  if (saveLeg) {
    return(list(col.matrix,p))
  } else {
    return(list(col.matrix,NULL))
  }
}
